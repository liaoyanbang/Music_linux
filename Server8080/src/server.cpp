...
{
	std::cout<<"Initializing evconnlistener"<<std::endl;
    base = event_base_new();
    ...
    listener = evconnlistener_new_bind(base,listener_cb,base,LEV_OPT_CLOSE_ON_FREE|LEV_OPT_REUSEABLE,10,
...
void PlayerServer::read_cb(struct bufferevent *bev, void *ctx)
{
    ...
    char cmd[32] = {0};
	strcpy(cmd, val["cmd"].asString().c_str());//get cmd
	...
	else if (!strcmp(cmd, "app_last"))      //上一首
	{
		p->player_operation(l, bev, cmd);
	}
	...
	else if (!strcmp(cmd, "app_vo_dowm")) //减小音量
	{
		p->player_operation(l, bev, cmd);
	}
	else if(!strcmp(cmd, "app_voice_set"))  //set voice
	{
		char voice[16] = {0};
		strcpy(voice, val["voice"].asString().c_str());//get cmd
		p->player_voice_set(l, bev, voice);
	} //
...
void PlayerServer::event_cb(struct bufferevent* bev, short event, void* arg)
{
